...
describe('Filter: lt operator', () => {
  ...
  describe('string that looks like a partial date (uses lexicographic order)', () => {
    ...
    // so it falls back to lexicographic comparison. Both '1026a' and '1027b' start with '1',
    // which is less than '2', so both match. If date comparison were used instead, both would
    // be excluded because they are not valid date strings.
    it('should use lexicographic comparison for bare year "2025"', () => {
      const users = [{ code: '1026a' }, { code: '1027b' }];
      expect(filterArray(users, 'code lt "2025"')).toEqual([{ code: '1026a' }, { code: '1027b' }]);
    });

    // "2025-10" is a year-month partial date and does not pass the full date prefix check (YYYY-MM-DD),
    // so it falls back to lexicographic comparison. Both '2024-01a' and '2024-01b' start with '2024'
    // which is lexicographically less than '2025', so both match. If date comparison were used instead,
    // both would be excluded because the trailing alphabetic character makes them invalid date strings.
    it('should use lexicographic comparison for year-month "2025-10"', () => {
      const users = [{ code: '2024-01a' }, { code: '2024-01b' }];
      expect(filterArray(users, 'code lt "2025-10"')).toEqual([{ code: '2024-01a' }, { code: '2024-01b' }]);
    });
  });
