...
describe('Filter: ew operator', () => {
  ...
  it('ew should be case insensitive firstName ew "AL"', () => {
    const users = [{ firstName: 'Michal' }];
    expect(filterArray(users, 'firstName ew "AL"')).toEqual([{ firstName: 'Michal' }]);
  });

  it('should return false when ew operator value is not a string', () => {
    const users = [{ firstName: 'Michal', lastName: 'Nieruchalski' }];
    expect(filterArray(users, 'firstName ew 123')).toEqual([]);
  });
