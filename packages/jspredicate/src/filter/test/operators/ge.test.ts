...
describe('Filter: ge operator', () => {
  describe('string (lexicographical)', () => {
    ...
    it('should return false when attribute value is not a string', () => {
      const users = [{ type: 30 }];
      expect(filterArray(users, 'type ge "25"')).toEqual([]);
    });
  });
  ...
  describe('integer (numeric)', () => {
    it('should return false when attribute value is less than the filter value', () => {
      const users = [{ age: 20 }];
      expect(filterArray(users, 'age ge 25')).toEqual([]);
    });

    it('should return true when attribute value is greater than the filter value', () => {
      const users = [{ age: 30 }];
      expect(filterArray(users, 'age ge 25')).toEqual([{ age: 30 }]);
    });

    it('should return true when attribute value is equal to the filter value', () => {
      const users = [{ age: 25 }];
      expect(filterArray(users, 'age ge 25')).toEqual([{ age: 25 }]);
    });
  ...
  describe('datetime (stored as string)', () => {
    describe('when filter value is a valid date', () => {
      it('should return false when attribute value is less than the filter value', () => {
        ...
        expect(filterArray(users, 'createdAt ge "2023-01-01T00:00:00Z"')).toEqual([]);
      });

      it('should return true when attribute value is greater than the filter value', () => {
        const users = [{ createdAt: '2025-01-01T00:00:00Z' }];
        expect(filterArray(users, 'createdAt ge "2023-01-01T00:00:00Z"')).toEqual([{ createdAt: '2025-01-01T00:00:00Z' }]);
      });

      it('should return true when attribute value is equal to the filter value', () => {
        const users = [{ createdAt: '2023-01-01T00:00:00Z' }];
        expect(filterArray(users, 'createdAt ge "2023-01-01T00:00:00Z"')).toEqual([{ createdAt: '2023-01-01T00:00:00Z' }]);
      });
  ...
  describe('string that looks like a partial date (uses lexicographic order)', () => {
    it('should use lexicographic comparison for bare year "2025"', () => {
      ...
      expect(filterArray(users, 'code ge "2025"')).toEqual([{ code: '2025a' }, { code: '2025b' }]);
    });

    it('should use lexicographic comparison for year-month "2025-01"', () => {
      const users = [{ code: '2025-01a' }, { code: '2025-01b' }];
      expect(filterArray(users, 'code ge "2025-01"')).toEqual([{ code: '2025-01a' }, { code: '2025-01b' }]);
    });
  });
  ...
  describe('unsupported filter values (always return false)', () => {
    it('should return false when filter value is true', () => {
      ...
      expect(filterArray(users, 'active ge true')).toEqual([]);
    });

    it('should return false when filter value is false', () => {
      const users = [{ active: false }];
      expect(filterArray(users, 'active ge false')).toEqual([]);
    });

    it('should return false when filter value is null', () => {
      const users = [{ age: 25 }];
      expect(filterArray(users, 'age ge null')).toEqual([]);
    });
  });
});
